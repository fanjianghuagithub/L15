...
int soundplus_init(int NrwFlag)
{
	...
	float PostGmin = 15.0f;    //db
#if 1
	int ret = 0;
	
	uint8_t key[] = {0x2A, 0x57, 0x77, 0x5D, 0x92, 0xE4, 0x15, 0xF9, 0xB5, 0x8C, 0xF9, 0x10, 0x73, 0xE9, 0x84, 0x98};
	//uint8_t key[] = {0xc8, 0x9b, 0x71, 0xa9, 0x7e, 0x8c, 0x90, 0x07, 0x80, 0x89, 0xc3, 0x6f, 0x76, 0xd6, 0xbd, 0xc5};
	ret = soundplus_auth(key, 16);
	TRACE(3,"<%s> ret=%d version=%s", __func__,ret, Sndp_get_alg_ver());

	ret = soundplus_auth_status();
	TRACE(3,"<%s> ret=%d 111version=%s", __func__,ret, Sndp_get_alg_ver());
#endif
